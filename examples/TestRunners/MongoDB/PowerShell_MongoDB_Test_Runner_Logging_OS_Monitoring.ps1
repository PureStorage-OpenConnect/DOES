############################################################################
#       MongoDB D.O.E.S PowerShell Test Runner Script V 0.1                #
#--------------------------------------------------------------------------#
#                                                                          #
############################################################################
...
# A database user with userAdminAnyDatabase and readWriteAnyDatabase       #
#                                  must exist                              #
############################################################################
...
$DeploymentType = "Standalone" # Use ReplicaSet if ReplicaSet.Use Standalone if Standalone
...
Function DropDatabaseTables()
{
    ...
    $commandset = {
                        param($Database, $Username, $Password, $Hostname, $TableAmplification, $Port, $DeploymentType)
                                    Import-Module 'C:\Program Files\Pure Storage\D.O.E.S\DOES.PowerShell.dll'
                                    Clear-DataEngine -DatabaseType MongoDB -Hostname $Hostname `
                                    -DatabaseName $Database -UserName $Username -Password $Password `
                                    -TableClearOperation Drop -TableAmplification $TableAmplification `
                                    -Port $Port -MongoDBDeploymentType $DeploymentType 
                  }
    ...
    for($DatabaseNumber = 1; $DatabaseNumber -le $NumberOfDatabases;$DatabaseNumber++)
    {
        ...
        $DatabaseSystems[$DatabaseSystemNumber], $TableCount, $PortTarget, $DeploymentType -AsJob
        $TestClientNumber++;
...
Function PopulateDatabase()
{
    ...
    $commandset = {
        param($Database, $Username, $Password, $Hostname, $TableAmplification, $DataSize, $DataSizeUnit,
                $DataFolder, $ThreadCount, $SchemaType, $PercentageRandomVal, $DeploymentType, `
                $TestName, $ObjectCategory, $Sequence)
                    Import-Module 'C:\Program Files\Pure Storage\D.O.E.S\DOES.PowerShell.dll'
                    $ObjectName = $ObjectCategory + "_" + $Database + "_" + $Username
                    Add-DataEngine -DatabaseType MongoDB -DatabaseName $Database -UserName $Username `
                    ...
                    -TableAmplification $TableAmplification -MongoDBDeploymentType $DeploymentType -LogData `
    ...
    for($DatabaseNumber = 1; $DatabaseNumber -le $NumberOfDatabases;$DatabaseNumber++)
    {
        ...
        $SchemaTemplate, $PercentageRandom, $DeploymentType, `
...
Function PerformDataEngineTest()
{
    ...
    if(!($DeferInitialWrite))
    {
        $commandset = {
            param($Database, $Username, $Password, $Hostname, $TableAmplification, $DataSize, $DataSizeUnit,
                    ...
                    $PercentageRandomVal, $SchemaType, $DeploymentType, $TestName, $ObjectCategory, $Sequence)
                        Import-Module 'C:\Program Files\Pure Storage\D.O.E.S\DOES.PowerShell.dll'
                        $ObjectName = $ObjectCategory + "_" + $Database + "_" + $Username
                        Test-DataEngine -DatabaseType MongoDB -Hostname $Hostname -DatabaseName $Database `
                        ...
                        -ChangeRate $ChangePercentage -MongoDBDeploymentType $DeploymentType -LogData `
                        -TestName $TestName -ObjectName $ObjectName -ObjectCategory $ObjectCategory -Sequence $Sequence
          }
      }
      else
            ...
            param($Database, $Username, $Password, $Hostname, $TableAmplification, $DataSize, $DataSizeUnit,
                  $ChangePercentage, $TestType, $DataFolder, $ThreadCount,
                    $PercentageRandomVal, $SchemaType, $DeploymentType, $TestName, $ObjectCategory, $Sequence)
                        ...
                        -DeferInitialWrite  -MongoDBDeploymentType $DeploymentType -LogData `
                        -TestName $TestName -ObjectName $ObjectName -ObjectCategory $ObjectCategory -Sequence $Sequence
          }
      }
    ...
    for($DatabaseNumber = 1; $DatabaseNumber -le $NumberOfDatabases;$DatabaseNumber++)
    {
        ...
        $TestType, $DataFolder, $ThreadCount, $PercentageRandom, $SchemaTemplate, $DeploymentType, `
        $TestName, $ObjectCategory, $Sequence -AsJob-AsJob
...
Function PerformDataEngineOLAPTest()
{
    ...
    $commandset = {
        param($Database, $Username, $Password, $Hostname, $TableAmplification, $DataSize, $DataSizeUnit,
               $ThreadCount, $DeploymentType, $TestName, $ObjectCategory, $Sequence)
                    Import-Module 'C:\Program Files\Pure Storage\D.O.E.S\DOES.PowerShell.dll'
                    $ObjectName = $ObjectCategory + "_" + $Database + "_" + $Username
                    Search-DataEngine -DatabaseType MongoDB -Hostname $Hostname -DatabaseName $Database `
                    -UserName $Username -Password $Password -Amount $DataSize -Unit $DataSizeUnit `
                    -NumberOfThreads $ThreadCount -TableAmplification $TableAmplification -QueryType UnionAll `
                    -MongoDBDeploymentType $DeploymentType -LogData `
                    -TestName $TestName -ObjectName $ObjectName -ObjectCategory $ObjectCategory -Sequence $Sequence
    }
    ...
    for($DatabaseNumber = 1; $DatabaseNumber -le $NumberOfDatabases;$DatabaseNumber++)
    {
        ...
        -ScriptBlock $commandset -ArgumentList $DatabaseName, $DatabaseUsername, $DatabasePassword, `
        $DatabaseSystems[$DatabaseSystemNumber], $TableCount, $DataSizePerUser, `
        $DataSizeUnit, $ThreadCount, $DeploymentType, $TestName, $ObjectCategory, $Sequence -AsJob
