...
namespace DOES.Cli
{
    public class Remove : Operation
    {
        ...
        private UInt64 _amountToDelete;
        ...
        public Remove(string IP, Dynamics.Database dbType, string databaseName, UInt64 amountToDelete, Dynamics.StorageUnit unit, string user, string password, bool useOracleSID, int numberOfThreads,
            ...
            Dynamics.MariaDBStorageEngine mariadbEngine, string ndbtablespace, Dynamics.MongoDBDeployment mongoDBDeployment , int portnumber, bool logData, string testName, string objectName, string ObjectCategory, int sequence)
        {
            ...
            _amountToDelete = amountToDelete;
            _unit = unit;
            _user = user;
            _password = password;
        ...
        {
            Dynamics.DataEngineOperation operation = Dynamics.DataEngineOperation.DeleteData;
            Mechanic serviceOperator = new Mechanic();
            //must ensure units are converted to bytes
            UInt64 amountToDeleteBytes = serviceOperator.ReturnDataUnitsAsBytes(_amountToDelete, _unit);
            DataEngineResults lastResults = new DataEngineResults(Dynamics.DataEngineOperation.DeleteData);
            ...
            try
            {
                if (_driver != null)
                {
                    ...
                    _task = Task.Run(() =>
                    {
                        ...
                        _controller.Delete(amountToDeleteBytes, capturedToken, null, null);
                    }, _token);
                }
                ...
                string topPbarMessage = "Overall progress for DOES.DataEngine delete operation                Amount processed : 0 Bytes of 0 Bytes";

                using (var progressBar = new ProgressBar(10000, topPbarMessage, options))
                {
                    var dataRateChild = progressBar.Spawn(10000, "Monitor data rate for data delete operations for " + dbtypeIdentifier + " Database : " + _dbname + " Data rate performance : 0 MB/s ");
                    var rowRateChild = progressBar.Spawn(10000, "Monitor row insertion rate for data delete operations row rate performance 0 Rows/s");
                    var transactionRateChild = progressBar.Spawn(10000, "Monitor transaction insertion rate for data delete operations transaction performance 0 Transactions/s");
                    ...
                    while (!_controller.GetResultsState() && !_token.IsCancellationRequested)
                    {
                        ...
                        lastResults = UpdateProgressBar(tempResults, duration, amountToDeleteBytes, lastResults,
        ...
        private void WriteFinalReport(DataEngineResultSet results, Analytic logObject)
        {
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "--                      DELETE OPERATION COMPLETED                 -- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "--------------------------------------------------------------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "---------------------------Overall Results--------------------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Data deleted                                  : " + amountOfDataProcessed + " " + storageUnit.ToString(), Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Rows deleted                                  : " + overallResults.ProcessedRows, Message.MessageType.Report));
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average data deleted per thread               : " + dataAddedPerThread + " " + storageUnit.ToString(), Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average rows deleted per thread               : " + rowsAddedPerThread, Message.MessageType.Report));
            ...
            for (int i = 0; i < results.ResultSet.Length; i++)
            {
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "Data deleted                                  : " + amountOfDataProcessedThisThread + " " + storageUnitThisThread.ToString(), Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Rows deleted                                  : " + results.ResultSet[i].ProcessedRows, Message.MessageType.Report));
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "--------------------------------------------------------------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "--                         END DELETE REPORT                       -- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "--------------------------------------------------------------------- ", Message.MessageType.Report));
        }
        #endregion
    }
