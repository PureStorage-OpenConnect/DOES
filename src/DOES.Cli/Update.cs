...
namespace DOES.Cli
{
    public class Update : Operation
    {
        ...
        private UInt64 _amountToUpdate;
        private Dynamics.StorageUnit _unit;
        private string _user;
        private string _password;
        private int _numberOfThreads;
        private bool _replaceWebPages;
        ...
        public Update(string IP, Dynamics.Database dbType, string databaseName, UInt64 amountToUpdate, Dynamics.StorageUnit unit, string user, string password, bool useOracleSID, int numberOfThreads,
            bool replaceWebPages, Dynamics.DatabaseSchema schema, int tableAmplifier, int columnWidth, string folderPath, double randomPercentage, string instanceName, string instanceNumber,
            ...
            Dynamics.MariaDBStorageEngine mariadbEngine, string ndbtablespace, Dynamics.MongoDBDeployment mongoDBDeployment, int portnumber, bool logData, string testName, string objectName, string ObjectCategory, int sequence)
        {
            ...
            _amountToUpdate = amountToUpdate;
            _unit = unit;
            _user = user;
            _password = password;
            _useOracleSID = useOracleSID;
            _numberOfThreads = numberOfThreads;
            _replaceWebPages = replaceWebPages;
            _schema = schema;
        ...
        {
            Dynamics.DataEngineOperation operation = Dynamics.DataEngineOperation.UpdateData;
            Mechanic serviceOperator = new Mechanic(_randomPercentage);

            //must ensure units are converted to bytes
            UInt64 amountToUpdateBytes = serviceOperator.ReturnDataUnitsAsBytes(_amountToUpdate, _unit);
            DataEngineResults lastResults = new DataEngineResults(Dynamics.DataEngineOperation.UpdateData);
            ...
            try
            {
                if (_driver != null)
                {
                    ...
                    if (_replaceWebPages)
                    {
                        if (_folderPath != null)
                        {
                            ...
                            _task = Task.Run(() =>
                            {
                                ...
                                _controller.Update(amountToUpdateBytes, capturedToken, _replaceWebPages, _folderPath, null, null);
                            }, _token);
                        }
                        else
                        {
                            Console.WriteLine("Update - Replacement selected but no folderpath for data file source provided use -Folder");
                            _messageQueue.AddMessage(new Message(DateTime.Now, "Replacement selected but no folderpath for data file source provided", Message.MessageType.Error));
                        }
                    }
                    else
                    {
                    ...
                    if (allChecksValid)
                    {
                        ...
                        string topPbarMessage = "Overall progress for DOES.DataEngine update operation                Amount processed : 0 Bytes of 0 Bytes";

                        using (var progressBar = new ProgressBar(10000, topPbarMessage, options))
                        {
                            var dataRateChild = progressBar.Spawn(10000, "Monitor data rate for data update operations for " + dbtypeIdentifier + " Database : " + _dbname + " Data rate performance : 0 MB/s ");
                            var rowRateChild = progressBar.Spawn(10000, "Monitor row insertion rate for data update operations row rate performance 0 Rows/s");
                            var transactionRateChild = progressBar.Spawn(10000, "Monitor transaction update rate for data update operations transaction performance 0 Transactions/s");
                            ...
                            while (!_controller.GetResultsState() && !_token.IsCancellationRequested)
                            {
                                ...
                                lastResults = UpdateProgressBar(tempResults, duration, amountToUpdateBytes, lastResults,
            ...
            ProgressBar pbar, ChildProgressBar dataRateBar, ChildProgressBar rowRateBar, ChildProgressBar transactionRateBar)
        {
            try
            {
                ...
                if (_logData)
                {
                    _stats.Enqueue(new InterimStat(results.ProcessedBytes, rowRatePerformance, transactionalPerformance,
                        dataRateperformanceMBs, results.ResultType, DateTime.Now));
                }
            }
        ...
        private void WriteFinalReport(DataEngineResultSet results, Analytic logObject)
        {
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "--                      UPDATE OPERATION COMPLETED                 -- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "--------------------------------------------------------------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "---------------------------Overall Results--------------------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Data updated                                  : " + amountOfDataProcessed + " " + storageUnit.ToString(), Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Rows updated                                  : " + overallResults.ProcessedRows, Message.MessageType.Report));
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "-------------------Overall Thread Aggregated Results----------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Data updated per thread               : " + dataAddedPerThread + " " + storageUnit.ToString(), Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Rows updated per thread               : " + rowsAddedPerThread, Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Transactions completed per thread     : " + transactionscompletedPerThread, Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Data rate performance per thread      : " + averageDataRatePerThread + " Mb/s", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Row rate performance per thread       : " + averageRowRatePerformancePerThread + " Rows/s", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Transactional performance per thread  : " + averageTransactionPerformancPerThread + " Transactions/s", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            ...
            for (int i = 0; i < results.ResultSet.Length; i++)
            {
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "------------------------Results for Thread " + (i + 1) + "------------------------ ", Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "Data updated                                  : " + amountOfDataProcessedThisThread + " " + storageUnitThisThread.ToString(), Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Rows updated                                  : " + results.ResultSet[i].ProcessedRows, Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Transactions completed                        : " + results.ResultSet[i].ProcessedTransactions, Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Transactions failed                           : " + results.ResultSet[i].FailedProcessedRows, Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Total seconds                                 : " + threadDuration.TotalSeconds, Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Data rate performance                         : " + dataRateperformanceThisThread + " Mb/s", Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Row rate performance                          : " + rowRatePerformanceThisThread + " Rows/s", Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Transactional performance                     : " + transactionalPerformanceThisThread + " Transactions/s", Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "--------------------------------------------------------------------- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "--                         END UPDATE REPORT                       -- ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "--------------------------------------------------------------------- ", Message.MessageType.Report));
        }
        #endregion
    }
