...
namespace DOES.Cli
{
    public class Search : Operation
    {
        ...
        private Dynamics.ReadQuery _queryType;
        ...
        public Search(string IP, Dynamics.Database dbType, string databaseName, UInt64 amountToQuery, Dynamics.StorageUnit unit, string user, string password, bool useOracleSID, int numberOfThreads,
         Dynamics.DatabaseSchema schema, int tableAmplifier, Dynamics.ReadQuery queryType,  int columnWidth, double randomPercentage, string instanceName, string instanceNumber,
        ...
        public override void ExecuteOperation()
        {
            ...
            try
            {
                ...
                string topPbarMessage = "Overall progress for DOES.DataEngine query operation                Amount processed : 0 Bytes of 0 Bytes";

                using (var progressBar = new ProgressBar(10000, topPbarMessage, options))
                {
                    var dataRateChild = progressBar.Spawn(10000, "Monitor data rate for data query operations for " + dbtypeIdentifier + " Database : " + _dbname + " Data rate performance : 0 MB/s ");
                    var transactionRateChild = progressBar.Spawn(10000, "Monitor query completion rate for  operations query performance 0 Query/s");
                    ...
                    while (!_controller.GetResultsState() && !_token.IsCancellationRequested)
                    {
                        ...
                        lastResults = UpdateProgressBar(tempResults, duration, amountToQueryBytes, lastResults,
                            progressBar, dataRateChild, transactionRateChild);
                        if (_logData)
                        {
        ...
        private DataEngineResults UpdateProgressBar(DataEngineResults results, TimeSpan span, UInt64 RequestedBytes, DataEngineResults oldResults,
            ProgressBar pbar, ChildProgressBar dataRateBar, ChildProgressBar transactionRateBar)
        {
            try
            {
        ...
        private void WriteFinalReport(DataEngineResultSet results, Analytic logObject)
        {
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average data queried per thread               : " + dataAddedPerThread + " " + storageUnit.ToString(), Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average operations completed per thread       : " + transactionscompletedPerThread, Message.MessageType.Report));
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average operational performance per thread    : " + averageTransactionPerformancPerThread + " Query/s", Message.MessageType.Report));
            _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
            ...
            for (int i = 0; i < results.ResultSet.Length; i++)
            {
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "Data queried                                  : " + amountOfDataProcessedThisThread + " " + storageUnitThisThread.ToString(), Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Operations completed                          : " + results.ResultSet[i].ProcessedTransactions, Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Operations failed                             : " + results.ResultSet[i].FailedProcessedRows, Message.MessageType.Report));
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "Operational performance                       : " + transactionalPerformanceThisThread + " Query/s", Message.MessageType.Report));
                _messageQueue.AddMessage(new Message(DateTime.Now, "                                                                ", Message.MessageType.Report));
