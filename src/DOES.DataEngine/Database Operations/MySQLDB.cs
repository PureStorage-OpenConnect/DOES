...
{
    /// <summary>
    /// This class handles MySQL database operations. 
    /// </summary>
    public class MySQLDB : DataVendor
    {
        ...
        private Dynamics.MySQLStorageEngine _engine;
        ...
        /// </summary>
        public MySQLDB(string hostname, string dbname, string username, string password, int amplifier,
            int port, Mechanic mechanic, Dynamics.MySQLStorageEngine storageEngine, MessageQueue messages)
        {
            _hostname = hostname;
        ...
        /// </summary>
        public MySQLDB(string hostname, string dbname, string username, string password, int amplifier,
            int port, Dynamics.DatabaseSchema schema, Mechanic mechanic, Dynamics.MySQLStorageEngine storageEngine, 
            string ndbTableSpace, MessageQueue messages)
        {
            _hostname = hostname;
            _databaseName = dbname;
            _userName = username;
        ...
        /// </summary>
        public MySQLDB(string hostname, string dbname, string username, string password, int port,
            Mechanic mechanic, Dynamics.MySQLStorageEngine storageEngine, MessageQueue messages)
        {
            _hostname = hostname;
            _databaseName = dbname;
            _userName = username;
                    ...
                    foreach (string s in baseSchemaObjects)
                    {
                        ...
                        else if (s.StartsWith("//") == true)
                        {
                            if (sb.Length != 0)
                            {
                                ...
                                SQLString = _mechanic.CreateMySQLTables(SQLString, _engine, _ndbTableSpace);
                ...
                catch (Exception ex)
                {
                    ...
                    foreach (string s in extensionSchemaObjects)
                    {
                        ...
                        else if (s.StartsWith("//") == true)
                        {
                            ...
                            {
                                string MySqlCommandString = sb.ToString();
                                MySqlCommandString = _mechanic.CreateMySQLTables(MySqlCommandString, _engine, _ndbTableSpace);
        ...
        {
            return Dynamics.Database.MySQL;
        }
        ...
        public override int InsertPointInTimeWrite()
        {
            ...
            using (MySqlConnection connection = GetConnection())
            {
                try
                {
                    connection.Open();
                    MySqlTransaction transaction = connection.BeginTransaction(System.Data.IsolationLevel.ReadCommitted);
                    MySqlCommand command = new MySqlCommand("INSERT INTO PointInTimeWrite(WrittenOn, WriteHash) VALUES (@WrittenOn, @WriteHash)", connection, transaction);
                ...
                try
                {
                    ...
                    for (int i = _mechanic.ReturnWebPageEncodingtables.GetLowerBound(0); i <= _mechanic.ReturnWebPageEncodingtables.GetUpperBound(0); i++)
                    {
                        for (int tableID = 0; tableID < _amplifier; tableID++)
                        {
                            ...
                            command = new MySqlCommand("DELETE FROM " + tableName + " WHERE PageID = " + PageID, connection);
                            command.Transaction = transaction;
                            command.ExecuteNonQuery();
        ...
        /// <summary>
        /// Forces a FLUSH tables scenario for MySQL
        /// </summary>
        ...
        /// <summary>
        /// For MySQL the CHECK Table option is run for each table in the database/schema
        /// </summary>
