...
{
    public class MicrosoftSQL_LogAnalytic : LogAnalytic
    {
        private Analytic _analytic;

        public MicrosoftSQL_LogAnalytic(Analytic analytic)
        {
            _analytic = analytic;
        }
        ...
        public override DOESTest LogTest()
        {
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    using (SqlCommand command = new SqlCommand())
                    {
                        ...
                        else
                        {
                            ...
                            if (_analytic.Test.CodeRevision == null)
                            {
                                ...
                                using (SqlCommand newtestEntryCommand = new SqlCommand())
                                {
                                    ...
                                    newtestEntryCommand.CommandType = CommandType.Text;
            ...
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    using (SqlCommand command = new SqlCommand())
                    {
                        ...
                        {
                            connection.Close();
                            using (SqlCommand newObjectEntryCommand = new SqlCommand())
                            {
                                ...
                                newObjectEntryCommand.CommandType = CommandType.Text;
                    ...
                    using (SqlCommand command = new SqlCommand())
                    {
                        ...
                        if (reader.HasRows == true)
                        {
                            ...
                            string updateSequenceObject = "UPDATE SequenceStats SET " + column + " = @NewValue WHERE TestID = (SELECT TestID FROM Tests WHERE TestName = '" + 
                                objectiveItem.AnalyticObject.Test.TestName + "') AND Sequence = @Sequence";
                            using (SqlCommand updateSequenceEntry = new SqlCommand())
                            {
                                ...
                                updateSequenceEntry.CommandType = CommandType.Text;
        ...
        public override void LogDataEngineStatsInterim(InterimStat stat)
        {
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    ...
                    using (SqlCommand command = new SqlCommand(newInterimStatString, connection))
                    {
            ...
            Dynamics.DataEngineOperation operation_, DateTime collectedOn)
        {
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    ...
                    using (SqlCommand command = new SqlCommand(newThreadStatString, connection))
                    {
                        command.CommandTimeout = 1200;
                        command.CommandType = CommandType.Text;
                        command.Parameters.AddWithValue("@ObjectID", _analytic.TestObject.ObjectID);
            ...
            Dynamics.DataEngineOperation operation, DateTime collectedOn)
        {
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    ...
                    using (SqlCommand command = new SqlCommand(newTotalStatsString, connection))
                    {
                        command.CommandTimeout = 1200;
                        command.CommandType = CommandType.Text;
                        command.Parameters.AddWithValue("@ObjectID", _analytic.TestObject.ObjectID);
        ...
        public override void LogDataEngineStatsFinalReport(string reportText, Dynamics.DataEngineOperation operation, DateTime collectedOn)
        {
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    ...
                    using (SqlCommand command = new SqlCommand(newFinalReportString, connection))
                    {
                        command.CommandTimeout = 1200;
                        command.CommandType = CommandType.Text;
                        command.Parameters.AddWithValue("@ObjectID", _analytic.TestObject.ObjectID);
            ...
            try
            {
                using (SqlConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection()) 
                {
                    string newResourceEntry = "INSERT INTO WindowsResourceData(ObjectID, Sequence, ProcessorTime, ProcessorPrivledgedTime, ProcessorInterruptTime, ProcessorDPCTime, " +
                        ...
                        "@PhysicalDiskAvgReadBytes, @PhysicalDiskAvgWriteBytes, @PhysicalDiskTime, @ProcessHandleCount, @ProcessThreadCount, @ProcessContextSwitchCount, @ProcessSystemCalls, @ProcessorQueueLength, " +
                        "@CollectedOn) ";
                    using (SqlCommand command = new SqlCommand(newResourceEntry, connection))
                    {
        ...
        {
            bool sucess = true;
            SqlConnection conn = _analytic.Persistence.PersistenceInstance.GetConnection();
            try
            {
                conn.Open();
                            ...
                            foreach (string s in baseSchemaCreationCommands)
                            {
                                ...
                                else if (s.StartsWith("//") == true)
                                {
                                    if (sb.Length != 0)
                                    {
                                        ...
                                        {
                                            if (!sqlex.Message.Contains("There is already an object named"))
                                            {
                                                sucess = false;
