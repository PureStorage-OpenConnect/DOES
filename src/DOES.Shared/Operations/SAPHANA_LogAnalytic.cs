...
{
    public class SAPHANA_LogAnalytic : LogAnalytic
    {
        private Analytic _analytic;

        public SAPHANA_LogAnalytic(Analytic analytic)
        {
            _analytic = analytic;
        }
        ...
        public override DOESTest LogTest()
        {
            try
            {
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    using (HanaCommand command = new HanaCommand())
                    {
                        ...
                        if (reader.HasRows == true)
                        {
                            ...
                            string updateTestObject = "UPDATE Tests SET LastChecked = ? WHERE TestName = '" + _analytic.Test.TestName + "'";
                            using (HanaCommand updateTestEntryCommand = new HanaCommand())
                            {
                                updateTestEntryCommand.Connection = connection;
                                updateTestEntryCommand.Parameters.AddWithValue("@LastChecked", TimeUpdated);
                        ...
                        else
                        {
                            ...
                            {
                                connection.Open();
                                HanaCommand testIDCommand = new HanaCommand("SELECT Seq_TestID.NEXTVAL FROM DUMMY", connection);
                                _analytic.Test.TestID = Convert.ToInt32(testIDCommand.ExecuteScalar());
                                connection.Close();
                                string newTestObject = "INSERT INTO TESTS (TestID, TestName, StartDate, LastChecked) VALUES(? ,?, ?, ?)";
                                using (HanaCommand newtestEntryCommand = new HanaCommand())
                                {
                                    newtestEntryCommand.Parameters.AddWithValue("@TestID", _analytic.Test.TestID);
                            ...
                            else
                            {
                                ...
                                _analytic.Test.TestID = Convert.ToInt32(testIDCommand.ExecuteScalar());
                                connection.Close();
                                string newTestObject = "INSERT INTO TESTS (TestID, TestName, StartDate, CodeRevision, LastChecked) VALUES(?, ?, ?, ?, ?)";
                                using (HanaCommand newtestEntryCommand = new HanaCommand())
                                {
                                    newtestEntryCommand.Parameters.AddWithValue("@TestID", _analytic.Test.TestID);
                                    newtestEntryCommand.Parameters.AddWithValue("@TestName", _analytic.Test.TestName);
                ...
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    using (HanaCommand command = new HanaCommand())
                    {
                        ...
                        if (_analytic.TestObject.ObjectCategory == null)
                        {
                            ...
                            newObject = "INSERT INTO Objects (TestID, ObjectID, ObjectTag) VALUES(?, ?, ?)";
                        }
                        else if (_analytic.TestObject.ObjectCategory != null)
                        {
                            ...
                            newObject = "INSERT INTO Objects (TestID, ObjectID,  ObjectTag, Category) VALUES(?, ?, ?, ?)";
                        }
                        command.CommandText = checkForExistingTestObject;
                        command.Connection = connection;
                        connection.Open();
                        ...
                        else
                        {
                            connection.Close();
                            connection.Open();
                            HanaCommand objectIDCommand = new HanaCommand("SELECT Seq_ObjectID.NEXTVAL FROM DUMMY", connection);
                            _analytic.TestObject.ObjectID = Convert.ToInt32(objectIDCommand.ExecuteScalar());
                            connection.Close();
                            using (HanaCommand newObjectEntryCommand = new HanaCommand())
                            {
                                newObjectEntryCommand.Parameters.AddWithValue("@TestID", _analytic.Test.TestID);
                                newObjectEntryCommand.Parameters.AddWithValue("@ObjectID", _analytic.TestObject.ObjectID);
                ...
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    using (HanaCommand command = new HanaCommand())
                    {
                        string checkForExistingTestObject = "SELECT * FROM SequenceStats WHERE TestID = (SELECT TestID FROM Tests WHERE TestName = '" +
                            objectiveItem.AnalyticObject.Test.TestName + "') AND Sequence = ?";
                        command.CommandText = checkForExistingTestObject;
                        ...
                        if (reader.HasRows == true)
                        {
                            ...
                            string updateSequenceObject = "UPDATE SequenceStats SET " + column + " = ? WHERE TestID = (SELECT TestID FROM Tests WHERE TestName = '" +
                                objectiveItem.AnalyticObject.Test.TestName + "') AND Sequence = ?";
                            using (HanaCommand updateSequenceEntry = new HanaCommand())
                            {
                        ...
                        else
                        {
                            ...
                            string newSequenceObject = "INSERT INTO SequenceStats (TestID, Sequence, " + column + ") VALUES ((SELECT TestID FROM Tests WHERE TestName = '" +
                                objectiveItem.AnalyticObject.Test.TestName + "'), ?, ?)";
                            using (HanaCommand newtestEntryCommand = new HanaCommand())
                            {
            ...
            try
            {
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    string newInterimStatString = "INSERT INTO DataEngineStatsInterim (ObjectID, Sequence, DataProcessedBytes, RowPerformancePerSecond, TransactionPerformancePerSecond, " +
                        "DataRatePerformanceMbytesPerSecond, Operation, CollectedOn) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    using (HanaCommand command = new HanaCommand(newInterimStatString, connection))
                    {
            ...
            try
            {
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    string newThreadStatString = "INSERT INTO DataEngineStatsThreads (ObjectID, Sequence, ThreadID, DataSizeBytes, RowsAffected, TransactionsCompleted, TransactionsFailed, TotalSeconds, " +
                        "Operation, CollectedOn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    using (HanaCommand command = new HanaCommand(newThreadStatString, connection))
                    {
                        command.CommandTimeout = 1200;
            ...
            try
            {
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    string newTotalStatsString = "INSERT INTO DataEngineStatsTotal (ObjectID, Sequence, DataSizeBytes, RowsAffected, TransactionsCompleted, TransactionsFailed, TotalSeconds, NumberOfThreads," +
                        "NumberOfTables, Operation, CollectedOn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    using (HanaCommand command = new HanaCommand(newTotalStatsString, connection))
                    {
                        command.CommandTimeout = 1200;
        ...
        public override void LogDataEngineStatsFinalReport(string reportText, Dynamics.DataEngineOperation operation, DateTime collectedOn)
        {
            try
            {
                ...
                {
                    connection.Open();
                    HanaCommand reportIDCommand = new HanaCommand("SELECT Seq_ReportID.NEXTVAL FROM DUMMY", connection);
                    ...
                    string newFinalReportString = "INSERT INTO DataEngineFinalReports (ReportID, ObjectID, Sequence, Report, Operation, CollectedOn) VALUES " +
                        "(?, ?, ?, ?, ?, ?)";
                    using (HanaCommand command = new HanaCommand(newFinalReportString, connection))
                    {
                        command.CommandTimeout = 1200;
            ...
            try
            {
                using (HanaConnection connection = _analytic.Persistence.PersistenceInstance.GetConnection())
                {
                    string newResourceEntry = "INSERT INTO WindowsResourceData(ObjectID, Sequence, ProcessorTime, ProcessorPrivledgedTime, ProcessorInterruptTime, ProcessorDPCTime, " +
                        ...
                        "CollectedOn) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ";
                    using (HanaCommand command = new HanaCommand(newResourceEntry, connection))
                    {
        ...
        {
            bool sucess = true;
            HanaConnection conn = _analytic.Persistence.PersistenceInstance.GetConnection();
            try
            {
                conn.Open();
                            ...
                            foreach (string s in baseSchemaCreationCommands)
                            {
                                ...
                                else if (s.StartsWith("//") == true)
                                {
                                    if (sb.Length != 0)
                                    {
                                        ...
                                        {
                                            if (!sqlex.Message.Contains("cannot use duplicate"))
                                            {
                                                sucess = false;
