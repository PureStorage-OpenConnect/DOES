...
{
    [Cmdlet(VerbsCommon.Remove, "DataEngine", DefaultParameterSetName = "MS SQL")]
    public class RemoveDataEngineCmdLet : Cmdlet
    {
        #region Parameters 
        ...
        private void WriteFinalReport(DataEngineResultSet results, Analytic logObject)
        {
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average data deleted per thread               : " + dataAddedPerThread + " " + storageUnit.ToString(), Message.MessageType.Info));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average rows deleted per thread               : " + rowsAddedPerThread, Message.MessageType.Info));
            ...
            for (int i = 0; i < results.ResultSet.Length; i++)
            {
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "Data deleted                                  : " + amountOfDataProcessedThisThread + " " + storageUnitThisThread.ToString(), Message.MessageType.Info));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Rows deleted                                  : " + results.ResultSet[i].ProcessedRows, Message.MessageType.Info));
