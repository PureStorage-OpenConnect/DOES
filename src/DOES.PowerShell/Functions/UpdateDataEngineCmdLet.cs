...
{
    [Cmdlet(VerbsData.Update, "DataEngine", DefaultParameterSetName = "MS SQL")]
    public class UpdateDataEngineCmdLet : Cmdlet
    {
        #region Parameters 
        ...
        public SwitchParameter ReplaceWebPages
        {
            get { return _replaceWebPages; }
            set { _replaceWebPages = value; }
        }
            ...
            try {
                if (_driver != null)
                {
                    ...
                    if (_replaceWebPages)
                    {
                        ...
                        {
                            Exception e = new Exception("Update - Replacement selected but no folderpath for data file source provided , use -Folder");
                            ErrorRecord record = new ErrorRecord(e, "No Folder Path", ErrorCategory.InvalidArgument, this);
                            WriteError(record);
                        }
                    }
                    else
        ...
        private void WriteFinalReport(DataEngineResultSet results, Analytic logObject)
        {
            ...
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Data updated per thread               : " + dataAddedPerThread + " " + storageUnit.ToString(), Message.MessageType.Info));
            _messageQueue.AddMessage(new Message(DateTime.Now, "Average Rows updated per thread               : " + rowsAddedPerThread, Message.MessageType.Info));
            ...
            for (int i = 0; i < results.ResultSet.Length; i++)
            {
                ...
                _messageQueue.AddMessage(new Message(DateTime.Now, "Data updated                                  : " + amountOfDataProcessedThisThread + " " + storageUnitThisThread.ToString(), Message.MessageType.Info));
                _messageQueue.AddMessage(new Message(DateTime.Now, "Rows updated                                  : " + results.ResultSet[i].ProcessedRows, Message.MessageType.Info));
